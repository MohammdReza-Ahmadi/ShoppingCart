...
namespace ShoppingCartApi.UseCases.Behaviors
{
        ...
        where TResponse : IRequest<TResponse>
    {
        ...
        {
            _logger.LogInformation("Start Request {@RequestName} {@DateTimeUtc} {@Request}",
                typeof(TRequest),DateTime.UtcNow,request);
            var result = await next();
            _logger.LogInformation("Start Request {@RequestName} {@DateTimeUtc} {@Response}",
    typeof(TRequest), DateTime.UtcNow, result);
            return result;
        }
    }
}
